...
- Shared responsive content frame refined for phones, tablets, foldables and split-screen.
- More consistent Material 3 light-mode contrast, outlines and surfaces.
- Branded animated loading state while the authenticated profile is loading.
- Admin top bar uses a cleaner surface treatment and consistent "Referral" wording.
- System status/navigation bars are aligned with the dark SevenGold visual identity.
- Core Google Sign-In, Cloudflare Worker and FCM flows are intentionally unchanged.
